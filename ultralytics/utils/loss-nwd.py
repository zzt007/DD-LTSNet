...
from .metrics import bbox_iou, probiou, nwd_bbox_iou, bbox_inner_mpdiou
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        ...
        nwd_iou = nwd_bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, NWDIoU=True)
        iou_ratio = 0.5
        lbox = ((1 - iou_ratio) * (1.0 - nwd_iou).mean() + iou_ratio * (1.0 - iou)).sum() / target_scores_sum
        loss_iou = ((iou.detach() * iou_ratio + nwd_iou.detach() * (1 - iou_ratio))).sum() / target_scores_sum

...
class RGB_HVI(nn.Module):
    ...
    def PHVIT(self, img):
        ...
        # 统一数据类型，确保与输入一致
        device = img.device
        dtype = img.dtype  # 获取输入的数据类型（可能是Half）
        ...
        # 使用与输入相同的数据类型创建零张量
        r = torch.zeros_like(h, dtype=dtype, device=device)
        g = torch.zeros_like(h, dtype=dtype, device=device)
        b = torch.zeros_like(h, dtype=dtype, device=device)
        
        hi = torch.floor(h * 6.0)
        f = h * 6.0 - hi
        # 确保中间变量p,q,t与输入数据类型一致
        p = (v * (1. - s)).to(dtype)
        q = (v * (1. - (f * s))).to(dtype)
        t = (v * (1. - ((1. - f) * s))).to(dtype)

...
if __name__ == "__main__":
    ...
    from .myEnhance.HVI_transform import RGB_HVI
    from ultralytics.nn.modules.myEnhance.DSAEN import dual_stream_net


class DSAEN(nn.Module):
    def __init__(self, in_channels=3, out_channels=3):
        super(DSAEN, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.dsnet = dual_stream_net(channel=32,
                          )
        
    def forward(self, x):
         # x shape [1,3,256,256]
        output4yolo = self.dsnet(x)
        return output4yolo
...
class InceptionModule(nn.Module):
    ...
    def forward(self, x):
        ...
        outputs = torch.cat([branch1x1, branch3x3, branch5x5, branch_pool], dim=1) # （1，4*4，64，64）
...
class InceptionModulev2(nn.Module):
    ...
    def _initialize_weights(self):
        ...
        nn.init.kaiming_normal_(self.branch1x1.weight, mode='fan_out', nonlinearity='relu')
        nn.init.kaiming_normal_(self.branch3x3.weight, mode='fan_out', nonlinearity='relu')
        nn.init.kaiming_normal_(self.branch3x3_1.weight, mode='fan_out', nonlinearity='relu')
        nn.init.kaiming_normal_(self.branch3x3_2.weight, mode='fan_out', nonlinearity='relu')
        nn.init.kaiming_normal_(self.branch_pool.weight, mode='fan_out', nonlinearity='relu')
        nn.init.kaiming_normal_(self.conv.weight, mode='fan_out', nonlinearity='relu')
    ...
    def forward(self, x):
        ...
        outputs = torch.cat([branch1x1, branch3x3, branch3x3_2, branch_pool], dim=1) # （1，4*4，64，64）
        outputs = self.conv(outputs)  # （1，1，64，64）
        return outputs
...
class SSF(nn.Module):
    def __init__(self, num_feats, encode_channels, target_channels, version="v2"):
        ...
        self.conv_fusion = Conv(num_feats, num_feats, 3, 1)
        ...
        self.final_conv = DWConv(target_channels * 2, target_channels, k=3, s=1)
...
class MSCA(nn.Module):
    def __init__(self, c1, upsample_type="sni"):
        ...
        # )
        self.final_conv2 = nn.Sequential(
            DWConv(c1 * 2, c1, k=3, s=1),  # 1x1 conv to change channels
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)  # upsample
        )
        # self.final_conv3 = nn.Sequential(
        #     DWConv(target_channels[2] * 2, 128, k=3, s=1),
        #     SNI(4)
        # )
        self.final_conv3 = nn.Sequential(
            DWConv(c1 * 4, c1, k=3, s=1),
            nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True)  # upsample
        )
        # self.final_conv4 = nn.Sequential(
        #     DWConv(target_channels[3] * 2, 128, k=3, s=1),
        #     SNI(8)
        # )
        self.final_conv4 = nn.Sequential(
            DWConv(c1 * 4, c1, k=3, s=1),
    ...
    def forward(self, x):
        ...
        final_featuremap_fused = torch.cat([P2_featuremap_fused, 
                                            P3_featuremap_fused, 
                                            P4_featuremap_fused, 
                                            P5_featuremap_fused], dim=1)
        final_featuremap_fused = self.softmax_conv(final_featuremap_fused)
        result = self.conv3x3(final_featuremap_fused)
